...
class ObjectiveEvaluationResults(NamedTuple):
    ...
    Attributes:
        objectives (Union[float, np.ndarray]): The objective function value/s
...
class ObjectiveBase(ABC):
    ...
    def evaluate(self, decision_vector: np.ndarray, use_surrogate: bool = False) -> ObjectiveEvaluationResults:
        ...
        Uses surrogate model if use_surrogates is true. If use_surrogates is
        False, uses func_evaluate which evaluates using the true objective
        function.

        Arguments:
            decision_vector (np.ndarray): A vector of Variables to be used in
                the evaluation of the objective.
            use_surrogate (bool) : A boolean which determines whether to use
                surrogates or true function evaluator. False by default.
...
class _ScalarObjective(ObjectiveBase):
    ...
    Attributes:
        ...
        maximize (List[bool]): List of boolean to determine whether the
    ...
    def _func_evaluate(self, decision_vector: np.ndarray) -> ObjectiveEvaluationResults:
        ...
        Returns:
            ObjectiveEvaluationResults: A named tuple containing the evaluated
                value, and uncertainity of evaluation of the objective function.
...
class VectorObjective(VectorObjectiveBase):
   ...
   Arguments:
        ...
        lower_bounds (Union[List[float], np.ndarray), optional): Lower bounds
            of the objective values. Defaults to None.
        upper_bounds (Union[List[float], np.ndarray), optional): Upper bounds
            of the objective values. Defaults to None.
        maximize (List[bool]): *List* of boolean to determine whether the
            objectives are to be maximized. All false by default
    ...
    def _func_evaluate(self, decision_vector: np.ndarray) -> ObjectiveEvaluationResults:
        ...
        Raises:
            ObjectiveError: When a bad argument is supplies to the evaluator
                or when the evaluator returns an unexpected number of outputs.
    ...
    def train(
        ...
        Arguments:
            model (BaseRegressor or List[BaseRegressors])
                A regressor or a list of regressors. The regressor/s, when
                initialized, should have a fit method and a predict method.
                The predict method should return the predicted objective
                value, as well as the uncertainity value, in a tuple. If the
                regressor does not support calculating uncertainity, return a
                tuple of objective value and None.
                If a single regressor is provided, that regressor is used for
                all the objectives.
                If a list of regressors is provided, and if the list contains
                one regressor for each objective, then those individual
                regressors are used to model the objectives. If the number of
                regressors is not equal to the number of objectives, an error
                is raised.
            model_parameters (Dict or List[Dict]): The parameters for the
