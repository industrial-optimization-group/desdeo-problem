    ...
    def evaluate(self, decision_vector: np.ndarray, objective_vector: np.ndarray) -> float:
        ...
        Returns:
            float: A float representing how and if the constraing has
                ...
                negative value represents a violation. The absolute value of
                the returned float functions as an indicator of the severity
                of the violation (or how well the constraint holds, if the
                returned value of positive).
...
class ScalarConstraint(ConstraintBase):
    ...
    def evaluate(self, decision_vector: np.ndarray, objective_vector: np.ndarray) -> float:
        ...
        This evaluates the constraint and return a float indicating how and
        if the constraint was violated. A negative value indicates a violation
        and a positive value indicates a non-violation.
